...
public class HomeController implements Initializable {
	...
	@FXML
	private MenuItem projectRefreshBtn;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		ObservableList<Task1> notificationlist = FXCollections
				.observableArrayList();
		...
		tmp.getStage().setOnCloseRequest(e -> {
			// before closing window sending msg to server that current user is
			// offline
		...
		// Task1
		// data=notificationListView.getSelectionModel().getSelectedItem();
		// System.out.println("select index is"+item+"\n data is"+data.getId());
		//
		// });

		System.out.println("home controller method called,user="
				+ tmp.getVuser().getUser_name());

		currentUserEmail.setText(tmp.getVuser().getUser_name());
		currentUserName.setText(tmp.getVuser().getUser_email());

		SingleSelectionModel<Tab> selectionModel = home_tabPane
				.getSelectionModel();
		...
		try {
			...
			out.println(tmp.getVuser().getUser_name());// sending username to
														// server
		...
		projectTree.setOnMouseClicked(e -> {
			if (e.getClickCount() == 2) {
				...
				TreeItem<ProjectModel> item = projectTree.getSelectionModel()
						.getSelectedItem();// getting selected treeitem
				// dont show root item in tabview ,only its children
				if (item.getChildren().size() == 0) {

					// true means: tab is already loaded in tabview so view that
					// tab without adding
					// in tabview again
					...
					Tab currentTab = null;
					for (Tab a : tabList) {
						// System.out.println("parent " + a.getId() + "name " +
						// a.getText());
						if (item.getParent().toString().equals(a.getId())
								&& item.getValue().getProject_name()
										.equals(a.getText())) {
							currentTab = a;
							flag = true;
							break;
					...
					if (flag) {
						System.out.println(
								"already exist changing active selectionn mode");
						// System.out.println("tab selected currnet" +
						// currentTab.getText());
						selectionModel.select(currentTab);// changin selected
															// tab to present
															// click tab
						...
						loader.setLocation(
								getClass().getResource("/application/"
										+ item.getValue().getproject_file()
										+ ".fxml"));
						try {
							loader.load();
						...
						if (item.getValue().getproject_file()
								.equals("ProjectInformation")) {
							System.out.println(
									"projectInformationController called 1");
							ProjectInformationController controller = loader
									.getController();

							controller.setData(
									Integer.valueOf(item.getParent().getValue()
											.getproject_file()),
									splitString(item.getParent().getValue()
											.getProject_name()));

						} else if (item.getValue().getproject_file()
								.equals("TeamMember")) {
							System.out
									.println("team member controller called 1");
							TeamMemberController controller = loader
									.getController();
							...
							controller.setData(
								...
								.equals("ProjectTask")) {
							System.out
									.println("ProjectTask controller called 1");
							ProjectTaskController controller = loader
									.getController();
							// passing projectName and user id
							controller.setData(
									Integer.valueOf(item.getParent().getValue()
								...
								.equals("TeamCalender")) {
							System.out.println(
									"TeamCalender controller called 1");
							TeamCalenderController controller = loader
									.getController();
							// passing projectName and user id
						...
						tab1.setContent(p);
						String rootItem = item.getParent().getValue()
								.getProject_name();
						Tooltip tooltip = new Tooltip(rootItem + "/"
								+ item.getValue().getProject_name());
						tab1.setTooltip(tooltip);
						home_tabPane.getTabs().add(tab1); // adding tab in
															// tabview
					}
				}
	...
	public void CreateButtonAction(MouseEvent e) {
		...
		FXMLLoader loader = new FXMLLoader(
				getClass().getResource("/application/ProjectCreation.fxml"));
		try {
			loader.load();
		} catch (IOException e1) {
			// TODO Auto-generated catch block
		...
		String css1 = this.getClass()
				.getResource("/application/application.css").toExternalForm();
		String css2 = this.getClass()
				.getResource("/application/ProjectCreation.css")
				.toExternalForm();
		scene.getStylesheets().addAll(css1, css2);
		Stage stage = new Stage();
		Stage previous_stage = (Stage) ((Node) e.getSource()).getScene()
				.getWindow();
	...
	public static void getProjectDetail() {
		...
		List<Project> data = ProjectDao
				.getProjectNameForTreeView(tmp.getVuser().getUser_id());
		...
		for (Project element : data) {
			...
			TreeItem<ProjectModel> node = new TreeItem<>(new ProjectModel(
					element.getProjectId() + "",
					element.getprojectTitle() + "[-> " + role_name + "]"));

			root.getChildren().add(node);
			TreeItem<ProjectModel> nodeA1 = new TreeItem<>(new ProjectModel(
					"ProjectInformation", "Project Information"));
			TreeItem<ProjectModel> nodeB1 = new TreeItem<>(
					new ProjectModel("TeamMember", "Team Member"));
			TreeItem<ProjectModel> nodeC1 = new TreeItem<>(
					new ProjectModel("ProjectTask", "Project Task"));
			TreeItem<ProjectModel> nodeD1 = new TreeItem<>(
					new ProjectModel("TeamCalender", "Team Calender"));
